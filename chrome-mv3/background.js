...
async function captureFullPageInner(tab, settings) {
  ...
  try {
    ...
    _lastPreviewUrl = await blobToDataUrl(vorschauBlob);
  } catch (e) {
...
async function runOnActiveTab(wahl) {
  ...
  if (_lastPreviewUrl) { revokeDownloadUrl(_lastPreviewUrl); }
  _lastPdfUrl = null;
  _lastPreviewUrl = null;
  _lastPages = 0;
  _lastSaved = false;
